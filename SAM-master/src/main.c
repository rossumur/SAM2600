...
// in js world
void dump_js(const char* name, const void* data, int len)
{
...
// text to phonemes
const char* text2phoneme(const char* src)
{
    static char dst[256];
    int i;
    int len = strlen(src);
    ...
    dst[len++] = '[';
    dst[len] = 0;
    printf("text2phoneme %s %s\n",src,dst);
    if (!TextToPhonemes((unsigned char *)dst))
        return 0;
    return dst;
}

//
int phoneme2sound(const char* src, char* dst)
{
    //SetInput((char*)src);
    //return SAMMain();
    return 0;
